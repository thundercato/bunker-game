...
export class BunkerV19Scene extends BunkerV18Scene {
  ...
  public override create(): void {
    ...
    this.runtimeV28().knifeLocation = "armed";
  ...
  public override update(time: number, delta: number): void {
    ...
    this.entrancePrompt.setVisible(
      !this.inTunnels && this.nearEntrance(player),
    );
    this.exitPrompt?.setVisible(this.inTunnels && this.nearExit(player));
    for (const room of this.rooms)
      room.prompt.setVisible(
        this.inTunnels &&
          !room.open &&
          this.distanceTo(player, room.door) <= DOOR_RANGE,
      );
    ...
    if (nearby && this.searchPrompt)
      this.searchPrompt
        .setPosition(nearby.body.x, nearby.body.y - 42)
        .setText(nearby.searched ? "SEARCHED" : "USE · SEARCH");
  ...
  private createEntranceDoor(): void {
    ...
    this.entrancePrompt = this.add
      ...
      .setDepth(50)
      .setVisible(false);
    if (this.tunnelPlayer) this.tunnelPlayer.setPosition(x, y - 90);
  }
  ...
  private nearEntrance(player: Phaser.Physics.Arcade.Sprite): boolean {
    return this.distanceTo(player, this.entranceDoor) <= DOOR_RANGE;
  }
  private nearExit(player: Phaser.Physics.Arcade.Sprite): boolean {
    return (
      Boolean(this.exitMarker) &&
      this.distanceTo(player, this.exitMarker!) <= EXIT_RANGE
    );
  }
  ...
  private enterTunnels(): void {
    ...
    camera.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      this.destroyTunnel();
      this.inTunnels = true;
      this.bunkerBounds = new Phaser.Geom.Rectangle(
        this.physics.world.bounds.x,
        this.physics.world.bounds.y,
        this.physics.world.bounds.width,
        this.physics.world.bounds.height,
      );
      this.tunnelOrigin.set(
        this.bunkerBounds.right + 640,
        this.bunkerBounds.top + 320,
      );
      this.generateTunnel();
      ...
      camera.once(Phaser.Cameras.Scene2D.Events.FADE_IN_COMPLETE, () => {
        ...
        this.toastV28("THE HATCH CLANGS SHUT BEHIND YOU");
      });
    });
  }
  ...
  private generateTunnel(): void {
    ...
    const deadEnds = this.findDeadEnds(grid).filter(
    ...
    for (let y = 0; y < ROWS; y++)
      for (let x = 0; x < COLS; x++) {
        ...
        if (grid[y]![x]) {
          ...
          root.add(
            this.add
              .rectangle(wx, wy, CELL, CELL, wet ? 0x26332e : 0x0c1113)
              .setStrokeStyle(1, wet ? 0x43564b : 0x151d20)
              .setDepth(1),
          );
          if (wet)
            root.add(
              this.add
                .ellipse(wx, wy, CELL * 0.75, CELL * 0.28, 0x526558, 0.3)
                .setDepth(2),
            );
        }
      }
    ...
    const rails = this.add
      .rectangle(0, 0, 34, 46, 0x1d2422)
      .setStrokeStyle(3, 0xa8b39e);
    const rungs = [-14, -4, 6, 16].map((ry) =>
      this.add.rectangle(0, ry, 25, 3, 0xb8c1ac),
    );
    this.exitMarker = this.add
      .container(exitX, exitY, [rails, ...rungs])
      .setDepth(12);
    this.exitPrompt = this.add
      .text(exitX, exitY - 52, "USE · CLIMB LADDER", {
        fontFamily: "monospace",
        fontSize: "12px",
        color: "#edffe6",
        backgroundColor: "#07100ddd",
      ...
      .setDepth(50)
      .setVisible(false);
    this.searchPrompt = this.add
      .text(0, 0, "USE · SEARCH", {
        fontFamily: "monospace",
        fontSize: "12px",
        color: "#ffe9b0",
        backgroundColor: "#151006dd",
        padding: { x: 7, y: 4 },
      })
      .setOrigin(0.5)
      .setDepth(60)
    ...
    for (const cell of openCells.slice(0, Phaser.Math.Between(28, 42)))
    ...
    for (let y = 1; y < ROWS - 1; y++)
      for (let x = 1; x < COLS - 1; x++)
        if (!grid[y]![x]) {
          ...
          ).filter(([dx, dy]) => !grid[y + dy]![x + dx]).length;
  ...
  private carveRoom(grid: boolean[][], seed: { x: number; y: number }): void {
    ...
    ).find(([dx, dy]) => !grid[seed.y + dy]![seed.x + dx]);
    if (!openDir) return;
    const [odx, ody] = openDir;
    const dx = -odx;
    const dy = -ody;
    const cx = seed.x + dx * 2;
    const cy = seed.y + dy * 2;
  ...
  private createRooms(
    ...
    for (const seed of seeds) {
      const x = this.tunnelOrigin.x + seed.x * CELL + CELL / 2;
      const y = this.tunnelOrigin.y + seed.y * CELL + CELL / 2;
      const slab = this.add
        .rectangle(0, 0, 38, 10, 0x5d4734)
        .setStrokeStyle(2, 0xa18563);
      const handle = this.add.circle(12, 0, 2, 0xd4b67d);
      const door = this.add.container(x, y, [slab, handle]).setDepth(9);
      const prompt = this.add
        .text(x, y - 38, "USE · OPEN ROOM", {
          fontFamily: "monospace",
          fontSize: "11px",
          color: "#ffe9b0",
          backgroundColor: "#151006dd",
      ...
      for (let i = 0; i < count; i++) {
        ...
        const shape = this.add
          .rectangle(
            0,
            0,
            30 + Phaser.Math.Between(0, 18),
            22 + Phaser.Math.Between(0, 18),
            spec.colour,
          )
          .setStrokeStyle(2, 0x171411);
        const label = this.add
          .text(0, 0, spec.label.split(" ")[0]!, {
            fontFamily: "monospace",
            fontSize: "7px",
            color: "#d8cfbf",
          })
        ...
        state.furniture.push(searchable);
        this.searchables.push(searchable);
        root.add(body);
      }
      this.rooms.push(state);
      root.add([door, prompt]);
    }
  }

  private nearestClosedRoom(
    player: Phaser.Physics.Arcade.Sprite,
  ): RoomState | undefined {
    return this.rooms
      .filter(
        (room) =>
          !room.open && this.distanceTo(player, room.door) <= DOOR_RANGE,
      )
      .sort(
        (a, b) =>
          this.distanceTo(player, a.door) - this.distanceTo(player, b.door),
      )[0];
  }
  ...
  private openRoom(room: RoomState): void {
    ...
    this.toastV28("ROOM OPENED · SEARCH EVERYTHING");
  }

  private nearestSearchable(
    player: Phaser.Physics.Arcade.Sprite,
  ): SearchableState | undefined {
    return this.searchables
      ...
      .sort(
        (a, b) =>
          Phaser.Math.Distance.Between(player.x, player.y, a.body.x, a.body.y) -
          Phaser.Math.Distance.Between(player.x, player.y, b.body.x, b.body.y),
      )[0];
  }

  private searchObject(item: SearchableState): void {
    if (item.searched) {
      this.toastV28("ALREADY SEARCHED");
      return;
    }
    this.searching = true;
    this.runtimeV28().uiOpen = true;
    const player = this.tunnelPlayer;
    if (player) (player.body as Phaser.Physics.Arcade.Body).setVelocity(0, 0);
    const overlay = document.querySelector<HTMLElement>(".game-overlay");
    if (!overlay) {
      this.searching = false;
      this.runtimeV28().uiOpen = false;
      return;
    }
    overlay.classList.add("is-open");
    const panel = document.createElement("div");
    panel.className = "message-panel search-panel";
    panel.innerHTML =
      "<h2>SEARCHING</h2><p>" +
      item.label +
      "</p><div class='search-progress'><i></i></div>";
    overlay.replaceChildren(panel);
    const bar = panel.querySelector<HTMLElement>(".search-progress i");
    if (bar) {
      bar.style.transitionDuration = String(item.duration) + "ms";
    ...
    this.time.delayedCall(item.duration, () => {
      ...
      this.grantLoot();
      item.body.setAlpha(0.45);
      this.toastV28(item.label + " SEARCHED");
    });
  }
  ...
  private grantLoot(): void {
    ...
    window.dispatchEvent(
      new CustomEvent("bunker-loot-found", { detail: { loot } }),
    );
    this.toastV28(loot === "NOTHING USEFUL" ? loot : "FOUND · " + loot);
  }
  ...
  private spawnEnemy(cellX: number, cellY: number): void {
    ...
    const kind: EnemyKind =
      roll < 0.48 ? "spider" : roll < 0.96 ? "rat" : "lurker";
    const x = this.tunnelOrigin.x + cellX * CELL + CELL / 2,
      y = this.tunnelOrigin.y + cellY * CELL + CELL / 2,
      key = "tunnel-" + kind;
    if (!this.textures.exists(key)) {
      const g = this.make.graphics({ x: 0, y: 0 });
      if (kind === "lurker") {
        g.fillStyle(0x5a625d, 1).fillRect(11, 4, 14, 24);
        g.fillCircle(18, 4, 7);
        g.lineStyle(3, 0x333936)
          .lineBetween(12, 24, 7, 31)
          .lineBetween(24, 24, 29, 31);
      } else {
        g.fillStyle(kind === "spider" ? 0x17110f : 0x6f6258, 1).fillEllipse(
          16,
          16,
          kind === "spider" ? 18 : 25,
          kind === "spider" ? 13 : 12,
        ...
        if (kind === "spider") {
          ...
          g.fillTriangle(4, 13, 10, 8, 11, 16);
          g.lineStyle(2, 0x9a8271, 1).lineBetween(28, 17, 35, 13);
        }
      }
    ...
    const collider = this.tunnelWalls
      ? this.physics.add.collider(sprite, this.tunnelWalls)
      : undefined;
    this.enemies.push({
      sprite,
      kind,
      health: kind === "spider" ? 1 : kind === "rat" ? 2 : 3,
      nextTurnAt: 0,
      collider,
    });
  }
  ...
  private updateEnemies(time: number, _delta: number): void {
    for (const enemy of this.enemies) {
      ...
      if (time >= enemy.nextTurnAt) {
        ...
        const speed =
          enemy.kind === "lurker" ? 18 : enemy.kind === "rat" ? 42 : 30;
  ...
  private checkEnemyContact(
    ...
    const runtime = this.runtimeV28();
    runtime.health = Phaser.Math.Clamp(runtime.health - ENEMY_DAMAGE, 0, 100);
    runtime.emitState();
    player.setTintFill(0xffffff);
    this.time.delayedCall(90, () => player.clearTint());
    this.time.delayedCall(180, () => player.setAlpha(0.25));
    this.time.delayedCall(270, () => player.setAlpha(1));
    const away = new Phaser.Math.Vector2(
      player.x - hit.sprite.x,
      player.y - hit.sprite.y,
    )
      .normalize()
      .scale(100);
    hit.sprite.setVelocity(-away.x, -away.y);
    this.toastV28(
      hit.kind.toUpperCase() + " ATTACK · -" + String(ENEMY_DAMAGE) + " HEALTH",
    );
    if (runtime.health <= 0) this.showDeath();
  }

  private readonly onGunshot = (): void => {
    if (!this.inTunnels || !this.tunnelPlayer) return;
    const origin = new Phaser.Math.Vector2(
        this.tunnelPlayer.x,
        this.tunnelPlayer.y,
      ),
      direction = this.lastFacing.clone().normalize();
  ...
  private damageEnemy(enemy: EnemyState, amount: number): void {
    ...
    this.toastV28(enemy.kind.toUpperCase() + " KILLED");
  }

  private toggleInventory(): void {
    const overlay = document.querySelector<HTMLElement>(".game-overlay");
    if (this.runtimeV28().uiOpen && overlay?.classList.contains("is-open")) {
      overlay.classList.remove("is-open");
      overlay.replaceChildren();
      this.runtimeV28().uiOpen = false;
      return;
    }
    this.runtimeV28().openBackpack?.();
  }
  ...
  private togglePauseMenu(): void {
    ...
    overlay.classList.add("is-open");
    const panel = document.createElement("div");
    panel.className = "message-panel pause-panel";
    panel.innerHTML =
      "<h2>PAUSED</h2><button class='resume-game'>RESUME</button><button class='restart-game'>RESTART</button>";
    panel
      .querySelector(".resume-game")
      ?.addEventListener("click", () => this.closePauseMenu());
    panel
  ...
  private showDeath(): void {
    ...
    overlay.classList.add("is-open");
    const panel = document.createElement("div");
    panel.className = "message-panel death-panel";
    panel.innerHTML =
      "<h2>YOU DIED</h2><p>The tunnels keep what they take.</p><button>RESTART</button>";
    panel
  ...
  private completeDemo(): void {
    ...
    panel.innerHTML =
      "<h2>DEMO COMPLETE</h2><p>You found the ladder out.</p><button>RETURN TO BUNKER</button>";
  ...
  private leaveTunnels(): void {
    ...
    camera.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
      ...
      this.destroyTunnel();
      if (this.bunkerBounds)
        this.physics.world.setBounds(
          this.bunkerBounds.x,
          this.bunkerBounds.y,
          this.bunkerBounds.width,
          this.bunkerBounds.height,
        );
  ...
  private destroyTunnel(): void {
    ...
    this.searchPrompt = undefined;
  }

  private toastV28(message: string): void {
    window.dispatchEvent(
