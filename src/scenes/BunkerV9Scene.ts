...
export class BunkerV9Scene extends BunkerV8Scene {
  ...
  private rackSlide(): void {
    ...
    if (magazine && magazine.rounds > 0) {
      ...
      this.toast("CLICK · NO ROUND AVAILABLE");
    }
  }
  ...
  private installFirearmStyles(): void {
    ...
    style.textContent = `
      .firearm-item-panel .firearm-art{margin:12px auto;width:min(560px,76vw);height:210px;display:grid;place-items:center;border:3px solid #66746c;background:radial-gradient(circle,#38413d,#101514 72%);font-size:96px}
      .firearm-item-panel .firearm-art.magazine{font-size:130px}.firearm-item-panel .firearm-art.ammo{font-size:120px;color:#d5a94c}
      .firearm-item-panel .item-actions{display:flex;flex-wrap:wrap;gap:10px;justify-content:center}.firearm-item-panel .item-actions button{min-width:150px;padding:13px 16px}
      .firearm-inventory .storage-cell span{font-size:9px}.firearm-inventory header p{letter-spacing:.08em}
    `;
