...
const ZONES: Zone[] = [
  {
    ...
    rect: new Phaser.Geom.Rectangle(2 * TILE, 3 * TILE, 18 * TILE, 14 * TILE),
    framed: true,
  },
  {
    name: "CENTRAL CORRIDOR",
    rect: new Phaser.Geom.Rectangle(20 * TILE, 7 * TILE, 18 * TILE, 6 * TILE),
    framed: false,
  },
  {
    name: "TRAINING ROOM",
    rect: new Phaser.Geom.Rectangle(38 * TILE, 3 * TILE, 20 * TILE, 15 * TILE),
    framed: true,
  },
  {
    name: "LOWER PASSAGE",
    rect: new Phaser.Geom.Rectangle(25 * TILE, 13 * TILE, 9 * TILE, 18 * TILE),
    framed: false,
  },
];
...
export class ScrollingBunkerScene extends Phaser.Scene {
  ...
  private makeTextures(): void {
    ...
    tile("metal", 0x273139, 0x596772, (g) => {
      ...
      for (const [x, y] of [
        ...
        g.fillStyle(0x9d7942).fillCircle(x, y, 1);
    });
  ...
  private isWall(tx: number, ty: number): boolean {
    ...
    for (const zone of ZONES) {
      ...
      const edge =
        lx < TILE ||
        ly < TILE ||
        lx >= zone.rect.width - TILE ||
        ly >= zone.rect.height - TILE;
      ...
      const opening =
        ...
        (zone.name === "CENTRAL CORRIDOR" &&
          ((lx < TILE && ly >= TILE && ly <= 4 * TILE) ||
            (lx >= zone.rect.width - TILE && ly >= TILE && ly <= 4 * TILE) ||
            (ly >= zone.rect.height - TILE &&
              lx >= 4 * TILE &&
              lx <= 12 * TILE))) ||
  ...
  private addFurniture(): void {
    ...
    this.add
      .rectangle(5.4 * TILE, 8.5 * TILE, 1.3 * TILE, 1.1 * TILE, 0xb2a797)
      .setDepth(5);
    ...
    for (const tx of [43, 48, 53, 57]) {
      this.add
        .rectangle(tx * TILE, 11 * TILE, 34, 72, 0x79502f)
        .setStrokeStyle(3, 0x2b190e)
        .setDepth(4);
      this.add
        .circle(tx * TILE, 9.7 * TILE, 11, 0xa46f3d)
        .setStrokeStyle(3, 0x2b190e)
        .setDepth(5);
    }
  }
