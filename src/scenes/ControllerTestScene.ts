...
export class ControllerTestScene extends Phaser.Scene {
  ...
  public create(): void {
    ...
    this.add
      ...
      .setOrigin(0.5);
    this.add
      .text(640, 112, `v${VERSION} • JOYPAD TEST`, {
        fontFamily: "monospace",
        fontSize: "20px",
        color: "#93c5fd",
      })
    ...
    this.status = this.add
      .text(640, 165, "WAITING FOR CONTROLLER", {
        fontFamily: "monospace",
        fontSize: "24px",
        color: "#fbbf24",
        fontStyle: "bold",
    ...
    this.device = this.add
      .text(640, 210, "Connect your controller and press any button", {
        fontFamily: "monospace",
        fontSize: "16px",
        color: "#cbd5e1",
        align: "center",
    ...
    this.add
      .text(
        640,
        660,
        "On iPhone, the browser may not reveal the controller until you press a button.",
        {
          fontFamily: "monospace",
          fontSize: "14px",
          color: "#94a3b8",
  ...
  public update(): void {
    ...
    const pressed = pad.buttons
      .map(
        (button) =>
          `${button.index}: ${button.value.toFixed(2)}${button.pressed ? "  PRESSED" : ""}`,
      )
      .join("\n");
    this.buttons?.setText(`BUTTONS\n\n${pressed || "None"}`);
  }
