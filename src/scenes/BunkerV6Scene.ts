...
export class BunkerV6Scene extends Phaser.Scene {
  ...
  private readonly items: StoredItem[] = [
    {
      ...
      details:
        "The paper is dry and the tobacco still smells usable. A small comfort from the old world, with a price attached.",
    ...
    },
    {
      ...
      details:
        "A compact ration with enough protein to take the edge off hunger. The packet is intact and aggressively preserved.",
      stats: [
        "Hunger: +18",
        "Thirst: -2",
        "Weight: 0.2 kg",
  ...
  public create(): void {
    ...
    window.addEventListener(
      "bunker-take-item",
      this.onTakeItem as EventListener,
    );
  ...
  private emitStorage(): void {
    window.dispatchEvent(
      new CustomEvent("bunker-storage-open", {
  ...
  private runPressed(): boolean {
    return (
      (this.gamepad()?.buttons[10]?.pressed ?? false) || this.keys.SHIFT.isDown
    );
  }
  ...
  private updateStamina(running: boolean, delta: number): void {
    ...
    this.stamina = running
      ? Math.max(0, this.stamina - STAMINA_DRAIN_PER_SECOND * seconds)
      : Math.min(100, this.stamina + STAMINA_REFILL_PER_SECOND * seconds);
    this.emitState();
  }
  ...
  private tryInteract(): void {
    ...
    if (interaction === "bunk")
      window.dispatchEvent(
        new CustomEvent("bunker-message", {
          detail: {
            title: "YOUR BUNK",
            text: "A narrow military cot under cold concrete.",
          },
        }),
      );
    if (interaction === "weapons")
      window.dispatchEvent(
        new CustomEvent("bunker-message", {
          detail: {
            title: "WEAPON-CLEANING TABLE",
            text: "No weapons. Only bore brushes, cotton patches, gun oil and old cleaning rods.",
          },
        }),
      );
  }
  ...
  private handleCloseInput(): void {
    const pressed =
      ...
      this.keys?.ESC?.isDown === true;
    if (pressed && !this.closeHeld && this.uiOpen)
      window.dispatchEvent(new Event("bunker-storage-close-request"));
